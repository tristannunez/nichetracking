...
thirty_year_avg <- function(brick.in, avg_yrs){
 for(yr in avg_yrs){
   ...
   print(yrnum:(yrnum+29))
   brick.window <- brick.in[[yrnum:(yrnum+29)]]  
...
writeRaster(atap.30yr.mean, filename="./Rdata_in_out/atap_30yr_mean", bandorder='BIL', overwrite=TRUE)

mabt.30yr.mean <- thirty_year_avg(brick.in=mabt.br, avg_yrs=(1895+30):1980)
writeRaster(mabt.30yr.mean, filename="./Rdata_in_out/mabt_30yr_mean", bandorder='BIL', overwrite=TRUE)

names(mabt.30yr.mean)<- 1925:1980
mabt.subset <- mabt.30yr.mean[[c(1,1:5*10)]]
animate(mabt.subset, pause = 1)

animate(mabt.30yr.mean, pause = .01)
?animate
